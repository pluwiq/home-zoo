...
module AnimalManager
  ...
  class << self
    ...
    def get_passport
      Services::FindAnimalService.new(cats:, dogs:).get_valid_passport
    end
    ...
    def edit_animal_by(passport:)
      passport ||= get_passport
      Services::FindAnimalService.new(cats:, dogs:).find_and_execute(passport:) do |animal|
        create_service = Services::CreateAnimalService.new(cats:, dogs:, existing_passports: [])

        animal.instance_variable_set(:@age, create_service.send(:validate_field, field: :age).to_i)
        animal.instance_variable_set(:@weight, create_service.send(:validate_field, field: :weight).to_f)
